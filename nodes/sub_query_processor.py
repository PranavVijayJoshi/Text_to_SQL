    ...
    messages = [
        SystemMessage(
            content=(
                ...
                "5. For date/year filters: if a 'data_year_range' CTE is available in the listed CTEs, "
                "you MUST use it for the year filter: "
                "WHERE EXTRACT(YEAR FROM <date_col>) = (SELECT data_year FROM data_year_range). "
                "Never use CURRENT_DATE when data_year_range is available.\n"
                "6. Use GROUP BY for any aggregation.\n"
                "7. For top-1-per-group, use ROW_NUMBER() OVER (PARTITION BY ... ORDER BY ... DESC) "
                "in a subquery, then filter WHERE row_num = 1. Never use LIMIT for per-group selection.\n"
                "8. Always qualify column names with their table or CTE name.\n"
                "9. If the intent says 'category_tree' or 'recursive', write a true WITH RECURSIVE query: "
                "RECURSIVE category_tree(category_id, category_name, root_id, root_name) AS "
                "(base case: SELECT leaf rows ... UNION ALL recursive case: JOIN parent). "
                "A flat SELECT from categories is NOT a recursive CTE.\n\n"
