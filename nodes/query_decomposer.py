    ...
    messages = [
        SystemMessage(
            content=(
                ...
                "8. Any node that finds the top/maximum value per group (e.g. top category per customer, "
                ...
                "9. If the query involves sub-categories, category hierarchy, or ancestor/descendant "
                ...
                "listed in that node's depends_on.\n"
                "10. Whenever the query involves a time filter (this year, current year, recent orders), "
                "add a node with id 'data_year_range' as the very first node that runs: "
                "SELECT MAX(EXTRACT(YEAR FROM <date_col>)) AS data_year FROM <date_table>. "
                "Every subsequent node that filters by year MUST reference data_year_range in its "
                "depends_on and use data_year_range.data_year for the year filter instead of CURRENT_DATE.\n\n"
